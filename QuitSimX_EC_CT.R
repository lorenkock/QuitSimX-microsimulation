...
# LK3 EC. Start 2015.. 
baseline_ec_seg <- read_delim("/Users/lkock/Desktop/Data/EC move/Cold Turkey/segmented_prevalence_baseline.txt", 
                              ",", col_names = T)

# LK3 EC. Start 2015.Intervention start 2016. 0.33 moved from OTC 
ec_ct_seg_0.33 <- read_delim("/Users/lkock/Desktop/Data/EC move/Cold Turkey/segmented_prevalence_EC_CT_0.33.txt", 
                          ",", col_names = T)

# LK3 EC. Start 2015.Intervention start 2016. 0.66 moved from OTC 
ec_ct_seg_0.66 <- read_delim("/Users/lkock/Desktop/Data/EC move/Cold Turkey/segmented_prevalence_EC_CT_0.66.txt", 
                          ",", col_names = T)

# LK3 EC. Start 2015.Intervention start 2016. 0.66 moved from OTC 
ec_ct_seg_all <- read_delim("/Users/lkock/Desktop/Data/EC move/Cold Turkey/segmented_prevalence_EC_CT_All.txt", 
                         ",", col_names = T)
...
# 0.33 moved 
# Baseline
baseline_ec33_overall <- ec_ct_seg_0.33
...
# 0.66
baseline_ec66_overall <- ec_ct_seg_0.66
...
# All
baseline_ec_all_overall <- ec_ct_seg_all

baseline_ec_all_overall$smok <- baseline_ec_all_overall$smoker*baseline_ec_all_overall$weight

baseline_ec_all_overall <- baseline_ec_all_overall %>% 
  group_by(year) %>%
  summarise(across(c(smok, weight), list(sum = sum)))

baseline_ec_all_overall$smoker <- baseline_ec_all_overall$smok_sum/baseline_ec_all_overall$weight_sum

View(baseline_ec_all_overall)
...
# EC scenario A. 0.33 moved from CT 

df_ec_c2de <- ec_ct_seg_0.33 %>%
  subset(sg == "C2" | sg == "D" | sg =="E")

# ABC1
df_ec_abc1 <- ec_ct_seg_0.33 %>%
  subset(sg == "AB" | sg == "C1")
...
ec_ct_a <- ggplot(data = df_merge_ec_a, aes(x = year, y = Estimate, color = subgroup)) +
...
df_ec_c2de <- ec_ct_seg_0.66 %>%
  subset(sg == "C2" | sg == "D" | sg =="E")

# ABC1
df_ec_abc1 <- ec_ct_seg_0.66 %>%
  subset(sg == "AB" | sg == "C1")
...
ec_ct_b <- ggplot(data = df_merge_ec_b, aes(x = year, y = Estimate, color = subgroup)) +
  geom_line(aes(colour = subgroup), size = 1.3) +
  scale_colour_manual(name = "State", values = rocket) +
...
df_ec_c2de <- ec_ct_seg_all %>%
  subset(sg == "C2" | sg == "D" | sg =="E")

# ABC1
df_ec_abc1 <- ec_ct_seg_all %>%
  subset(sg == "AB" | sg == "C1")
...
ec_ct_c <- ggplot(data = df_merge_ec_c, aes(x = year, y = Estimate, color = subgroup)) +
  geom_line(aes(colour = subgroup), size = 1.3) +
  scale_colour_manual(name = "State", values = rocket) +
...
plot_figure <- ggarrange(ec_ct_a, ec_ct_b, ec_ct_c,
                         common.legend = TRUE, widths = c(1, 1),
                         labels = c("", "", ""), hjust = -0.0, vjust = 1.0,
                         ncol = 1, nrow = 3)

ggsave("Figure_ct_ec.png", plot_figure, width = 10, height = 20, dpi = 300)
...
# NORTH
df_ec_n <- ec_ct_seg_0.33 %>%
  subset(region == "NORTH EAST" | region == "NORTH WEST")

# LONDON
df_ec_lon <- ec_ct_seg_0.33 %>%
  subset(region == "LONDON")
...
ec_ct_a_reg <- ggplot(data = df_merge_ec_a, aes(x = year, y = Estimate, color = subgroup)) +
  geom_line(aes(colour = subgroup), size = 1.3) +
  scale_colour_manual(name = "State", values = rocket) +
  ggtitle("A) 1/3 moved to e-cigarettes") + 
  xlab('Year') + ylab('Smoking prevalence (%)') +
...
df_ec_n <- ec_ct_seg_0.66 %>%
  subset(region == "NORTH EAST" | region == "NORTH WEST")

# ABC1
df_ec_lon <- ec_ct_seg_0.66 %>%
  subset(region == "LONDON")
...
ec_ct_b_reg <- ggplot(data = df_merge_ec_b, aes(x = year, y = Estimate, color = subgroup)) +
  geom_line(aes(colour = subgroup), size = 1.3) +
  scale_colour_manual(name = "State", values = rocket) +
  ggtitle("B) 2/3 moved to e-cigarettes") + 
  xlab('Year') + ylab('Smoking prevalence (%)') +
...
df_ec_n <- ec_ct_seg_all %>%
  subset(region == "NORTH EAST" | region == "NORTH WEST")

# ABC1
df_ec_lon <- ec_ct_seg_all %>%
  subset(region == "LONDON")
...
ec_ct_c_reg <- ggplot(data = df_merge_ec_c, aes(x = year, y = Estimate, color = subgroup)) +
  geom_line(aes(colour = subgroup), size = 1.3) +
  scale_colour_manual(name = "State", values = rocket) +
...
plot_figure <- ggarrange(ec_ct_a_reg, ec_ct_b_reg, ec_ct_c_reg,
                         common.legend = TRUE, widths = c(1, 1),
                         labels = c("", "", ""), hjust = -0.0, vjust = 1.0,
                         ncol = 1, nrow = 3)

ggsave("Figure_ct_ec_reg.png", plot_figure, width = 12, height = 20, dpi = 300)
